...
def test_multiple_edges(logger):
    G = GraphWithAnomaly([(0,2),(1,4), (2,3), (3,2),(4,2), (5,3)], 1, 4, 3, 0, logger)

    # GNM anomaly
    #G._generate_anomaly()
    G.G_anomalies = []
    G.G_anomalies.append(GNM(G.n_anomaly, G.m_anomaly, logger))
    G.G_anomalies[0].graph = Graph(edges=[(0,1), (1,2), (2,3)],
            nodes={0,1,2,3}, is_sorted=False, degrees=None, logger=logger)
    ...
    G.G_normal.graph = Graph(edges=[(0,1), (1,4), (2,5), (3,5), (4,5)], nodes={0,1,2,3,4,5},
            is_sorted=False, degrees=None, logger=logger)

    multiple_edges = G._check_multiple_edges()
    assert multiple_edges == [((0,1), (0, (0,1)))]
